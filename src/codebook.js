...
export const getCodebookItemName = (codebookCode, codebookItemCode) => {
  ...
  if (codebook) {
    const codebookItem = codebook.find(
      (item) => item.code === codebookItemCode
    );
...
export const getCodebookRadioButtons = (
  codebookCode,
  checkedValue,
  radioOnChange
) =>
...
const codebooks = {
  ...
  HOUSE: [
    {
      code: "GRYFFINDOR",
      names: { cs: "Nebelvír", en: "Gryffindor" },
      order: 1,
    },
    {
      code: "HUFFLEPUFF",
      names: { cs: "Mrzimor", en: "Hufflepuff" },
      order: 2,
    },
    {
      code: "RAVENCLAW",
      names: { cs: "Havraspár", en: "Ravenclaw" },
      order: 3,
    },
